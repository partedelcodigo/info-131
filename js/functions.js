    ...
    for ( var i = 0; i < rows.length; i++ ) {
	   ...
	   /*if ( checkbox ) {*/
		   checkbox.onclick = function() {
				var rows = document.getElementsByTagName('tr');
                var index;
				//rows[i].className.substr(0,3)
				var index = parseInt(this.id.substr(13,1));
				//index=index+1;
				 if(this.checked)// = ! this.checked;
				 {	//alert('si'+index);
					rows[index].className += ' marked';
					sw=0;
            		return 1;
				 }
				 else
				 {	//alert('no'+index);
					rows[index].className = rows[index].className.replace(' marked', '');
					sw=1;
				 	return 0;
				 }
			}
		...
		//x.onmousedown = function() {
            ...
            if ( checkbox && checkbox.disabled == false ) {
                checkbox.checked = marked_row[unique_id]; //aqui marca el check
            }
        }*/
		//////////////////ee
		//rows[i] = aux;
